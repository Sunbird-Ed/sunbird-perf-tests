...
echo "csvFileProgram = " ${csvFileProgram}
echo "csvFileToken = " ${csvFileToken}
echo "platformSolutionsApi = " ${platformSolutionsApi}
...
sed "s#PROGRAM_FILE#${csvFileProgram}#g" $JMX_FILE_PATH > jmx.tmp
mv jmx.tmp $JMX_FILE_PATH
...
sed "s#PATH_PREFIX#${platformSolutionsApi}#g" $JMX_FILE_PATH > jmx.tmp
mv jmx.tmp $JMX_FILE_PATH
