...
echo "csvFileProject = " ${csvFileProject}
echo "csvFileToken = " ${csvFileToken}
echo "createObservationTaskApi = " ${createObservationTaskApi}
...
sed "s#PROJECT_FILE#${csvFileProject}#g" $JMX_FILE_PATH > jmx.tmp
mv jmx.tmp $JMX_FILE_PATH
...
sed "s#PATH_PREFIX#${createObservationTaskApi}#g" $JMX_FILE_PATH > jmx.tmp
mv jmx.tmp $JMX_FILE_PATH
