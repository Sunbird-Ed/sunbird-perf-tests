...
accessToken=$(curl -s -X POST https://dev.sunbirded.org/auth/realms/sunbird/protocol/openid-connect/token  -H 'content-type: application/x-www-form-urlencoded'  --data "client_id=admin-cli&username=${username}&password=password&grant_type=password" | jq -r '.access_token') # X-AUTHENTICATED-TOKEN
