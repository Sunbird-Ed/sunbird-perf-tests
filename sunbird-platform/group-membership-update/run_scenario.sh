...
echo "csvUserData = " ${csvUserData}
echo "groupMembershipUpdateApi = " ${groupMembershipUpdateApi}
...
sed "s#CSV_USERDATA#${csvUserData}#g" $JMX_FILE_PATH > jmx.tmp
mv jmx.tmp $JMX_FILE_PATH

sed "s#PATH_PREFIX#${groupMembershipUpdateApi}#g" $JMX_FILE_PATH > jmx.tmp
mv jmx.tmp $JMX_FILE_PATH
