...
csvFileHost=${9}
csvFileRequest=${10}
csvFromDataRequest=${11}
accountMergeApi=${12}


JMETER_HOME=/mnt/data/benchmark/apache-jmeter-5.3
...
echo "csvFromDataRequest = " ${csvFromDataRequest}
echo "accountMergeApi = " ${accountMergeApi}
...
sed "s#CSV_FROMDATA_REQUEST#${csvFromDataRequest}#g" $JMX_FILE_PATH > jmx.tmp
mv jmx.tmp $JMX_FILE_PATH

sed "s#PATH_PREFIX#${accountMergeApi}#g" $JMX_FILE_PATH > jmx.tmp
mv jmx.tmp $JMX_FILE_PATH
