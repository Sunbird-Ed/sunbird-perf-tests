...
csvFileHost=${11}
csvFileRequest=${12}
userSearchEmailApi=${13}

# Generating x-authenticated-token
accessToken=$(curl --location --request POST 'https://loadtest.ntp.net.in/auth/realms/sunbird/protocol/openid-connect/token' --header 'Content-Type: application/x-www-form-urlencoded' --data-urlencode 'client_id=mobilelogins' --data-urlencode 'grant_type=password' --data-urlencode 'client_secret=d023d75b-1983-4974-ae83-a85998e1292c' --data-urlencode 'username=loadtestadmin@yopmail.com' | jq -r '.access_token') # X-AUTHENTICATED-TOKEN
...
echo "userSearchEmailApi = " ${userSearchEmailApi}
...
sed "s#PATH_PREFIX#${userSearchEmailApi}#g" $JMX_FILE_PATH > jmx.tmp
mv jmx.tmp $JMX_FILE_PATH
