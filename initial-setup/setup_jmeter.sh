...
echo -e "\n\e[0;32m${bold}Installing openjdk8 and openjfx and open jdk 8. Please enter sudo password.${normal}"
sudo apt install openjdk-8-jdk
sudo apt install openjfx

echo -e "\n\e[0;32m${bold}Updating run_scenario.sh with Jmeter cluster info${normal}"
sed -i "s/JMETER_CLUSTER_REPLACE/$JMETER_CLUSTER/g" $JMETER_HOME/scripts/run_scenario.sh
